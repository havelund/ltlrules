...
\klaus{You above introduce some concepts before they really are explained. I wonder whether the reader will understand. Specifically the E, but also the Q. Why not just introduce as we go along?}
...
%\klaus{I think this lemma needs to tie the trace to the formula. Doron: I do not understand this comment.}
...
Runtime verification of temporal temporal specification often
...
\noindent {\bf Evaluation order.} The order of calculating $\current$ is subtle here, because
...
We defined models for FLTL based on {\em temporal} relations~\cite{Chomicki}. That is,
...
We now extend FLTL into EFLTL in a way that was motivated by the the propositional extension from LTL (PLTL) to ELTL (PELTL). We thus allow the following formula:
\begin{equation}
\label{EFLTL}
...
%\klaus{Something seems wrong above.}
