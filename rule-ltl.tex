...
\includegraphics[width=3.5in]{PROP.jpg}
...
restrictions to the state $s_i$, $p[i]$ and $q[i]$, respectively, are unary relations $p$ and $q$. The
...
$\mathit{inherent}(x , y)$ appears
in a state, denoting that a process 
$y$ wants to inherent variables from a process $x$,
there were a chain of states with process spawning
$\mathit{spawn}(x, x_1)$, $\mathit{spawn} (x_1, x_2)$ $\ldots$
$\mathit{spawn}(x_l , y)$. The reason
...
\footnote{The obtained model will have, in addition
...
\end{itemize}
Consequently, quantification over relations effectively extends the model ${\cal R}$ into a model ${\cal R}'$ within the scope of the quantifier. Note that quantification here is dynmaic (as in QLTL and QPLTL), since the relations are temporal and can have different sets of tuples in different states.
...
\end{theorem}
The proof of this theorem, which includes the encoding of a nontrivial property, and a combinatorial argument, appears in an appendix.
...
${\cal R}$,
where the last parameter is restricted to  $i$. A typical use
...
some state (corresponding to $\old$ and $\current$, which were used with propositional values in Section~\ref{LTLruntime});  these also form a relation over the domains of these variables (having some fixed order).
...
of specification, described in~(\ref{EFLTL}).
