...
input format of the tool, which are not expressible in the original first-order logic of \dejavu{} presented in \cite{HPU}. The first property \idsl{telemetry1} is a variant of
...
The indexing scheme is guided by an enumeration of the annotated abstract syntax tree of the formula, shown in Figure \ref{fig:ast}. The figure (generated by \dejavu) points out (blue node 5) the top node for the rule \idsl{closed(x)}, and calls to the rule (purple nodes 2 and 9).
